...
var markerClusterOptions = {
    ...
    gridSize: 45,
    styles: [{
      url: pathToFiles+regionName+"-marker-cluster.png",
      width: 53,
      height: 52,
      anchorText: [1,0],
      fontFamily: '"Frutiger W01", Helvetica, Arial, sans-serif',
      fontWeight: 'normal',
      textColor: '#ffffff',
      textSize: 14
    }]
...
var arrData = [];
var regionName, brandColour, pathToFiles, isResponsive;   // These variables are set using the vivify.js script.
...
function loadMapData(regionName, targetElement) {
  ...
  markerCluster = new MarkerClusterer(map, arrMarkers, markerClusterOptions);
  setFilters();
  applyFilters();
  ...
  // Add an event listener to redraw the window if the map resizes
  if (isResponsive) {
